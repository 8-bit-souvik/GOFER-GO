...
router.get('/discover', verify, function (req, res) {
    ...
    if (req.headers.verified) {
        ...
        members
            ...
            .select(' name , github_ID , profile_img , login_ID , about ')
            .exec(temp = (err, people) => {

                const chooseRandom = (people, num = 1) => {
                    const res = [];
                    for (let i = 0; i < num;) {
                        const random = Math.floor(Math.random() * people.length);
                        if (res.indexOf(people[random]) !== -1) {
                            continue;
                        };
                        res.push(people[random]);
                        i++;
                    };
                    return res;
                };
                // console.log(chooseRandom(people, 5));

                // console.log(people);

                res.send({ discover: chooseRandom(people, 5)});
